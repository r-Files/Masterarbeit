...
\end{split}
\end{equation}
The neuron takes the three input values $x_1, x_2 \text{ and } x_3$ and calculates the weighted sum based on the weights $w_1, w_2 \text{ and } w_3$. A bias $b$ is first added to this weighted sum and then the activation function $g$ is applied which finally gives the output of the neuron. The purpose of the activation function can be seen as a gate. Since the values of $z$ can lie in the interval (-$\infty$, $\infty$) after the linear transformation, a way must be found to determine whether the next neuron should be activated or not. For this reason neural networks use non-linear activation functions. Those activation functions can limit the value range and also enable the network to learn complex data structures. Some of the most commonly used activation functions in neural networks are: 
