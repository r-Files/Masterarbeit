...
This step, where the neural network takes an input vector and transforms it to a prediction is the first step to train the network and is called forward propagation \cite{Propagation}. The process shown in the figure (\ref{fig:NN_overview}) shows that after the generation of the predictions $\hat{y}$ the deviation of these to the real outputs $y$ must be measured. 

\begin{remark}
	Let $x^{(i)}$ and $y^{(i)}$ be the vector of the input and the real output values respectively. Let $W$ be the matrix of weights and $b$ the vector of biases used for the forward propagation. If $\hat{y}_{W,b}^{(i)}$ denotes the estimated outputs from the network then the loss score is given by:
	
	\begin{equation}
		L(b, W, x^{(i)}, y^{(i)}) = \frac{1}{2} \norm{\hat{y}_{W,b}^{(i)} - y^{(i)}}^2_2
	\end{equation}
\end{remark}
