...
The main loop starts with searching for the index of the gradient that is not yet present in the set $P$ and has the highest value. After this index has been moved from the set $R$ to the set $P$, the solution of the least square problem is calculated in step e), limited to the columns of the matrix $A$ whose indices occur in the set $P$. If all components of this least square solution are positive, the solution vector $x$ is overwritten with $s$ and after a new calculation of the gradient the main loop is started again. 
