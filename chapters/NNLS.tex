...
\begin{corollary}[\cite{AnaII}]\label{eq:Neumann}
	...
	\end{equation}  
\end{corollary}
It therefore follows by applying (\ref{eq:simplify}) and (\ref{eq:Neumann}) that:
\begin{align*}
	(A + E)^{-1} 	&= (I + A^{-1}E)^{-1}A^{-1} \\
...
\end{align*}
The relative error caused by the implementation of the perturbation of matrix $A$ has therefore an upper bound which is given by: 
