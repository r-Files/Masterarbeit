...
\end{equation*}
Where in the second step the definition of the Dot product was used. The vector $w$ is then characterized by the length and the direction: 
\begin{equation*}
	w  
	= \frac{-x^\top v}{\lVert v \rVert} \frac{v}{\lVert v \rVert}
	= -v \frac{x^\top v}{v^\top v}
\end{equation*}
The mirrored vector $Px$ can now be defined as:
\begin{equation*}
	Px   
	= x + 2w
...
\end{equation*}
The matrix constructed to achieve the linear mapping described above is called the Householder matrix  and is represented by the following matrix: 
\begin{equation}\label{eq:Householder}
H = I - 2 \frac{v v^\top}{v^\top v}
\end{equation}
Where $I$ in equation (\ref{eq:Householder}) is the identity matrix.
