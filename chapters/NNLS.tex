...
After analyzing how the runtimes of the different approaches behave in relation to the number of columns, there is still a need to analyze their accuracy. As mentioned above, methods two, three and seven scale best with an increasing number of columns of matrix $A$. Since method seven uses a $QR$ decomposition, methods two and three do not use it, an important aspect is to compare their accuracy. Graphic (\ref{fig:avg_errors}) shows how much the calculated solutions $s_{LS}$ deviate on average in absolute values from the actual values $s_{true}$.
\begin{figure}
	\centering
