...
\begin{definition}
	Let $A \in \R^{m \times n}$ with $m \geq n$ be a matrix. The decomposition of $A$ into a product $A = QR$ with an orthogonal matrix $Q \in \R^{m \times m}$ and an upper triangular matrix $R \in \R^{m \times n}$ is called a QR-decomposition. 
\end{definition}

\begin{remark} Let $Q \in R^{n \times n}$ be an orthogonal matrix an $x \in \R^n$ a vector, then the length of $x$ does not change when transformed by $Q$. 
	\begin{align*}
		\lVert Qx \rVert_2^2 = (Qx)^T (Qx) = x^T Q^T Q x = x^T I x = \lVert x \rVert_2^2 
	\end{align*}
\end{remark}
